...
void waiter(uintptr_t id)
{
    ...
    std::cout << "[Multiprocessor] Single CV: Waiter " << id << " finished running" << std::endl;
    mutex1.unlock();
}
...
void signaler(uintptr_t id)
{
    ...
    cv1.broadcast();
    std::cout << "[Multiprocessor] Single CV: Signaler " << id << " finished signaling" << std::endl;
    mutex1.unlock();
}

void thread_manager(uintptr_t)
{
    std::cout << "[Multiprocessor] Single CV: Running test" << std::endl;
    ...
    std::cout << "[Multiprocessor] Single CV: All threads have finished." << std::endl;
}
