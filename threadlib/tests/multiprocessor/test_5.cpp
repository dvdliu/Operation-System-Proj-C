...
void thread3(uintptr_t)
{
    ...
    assert(local_sum == 20);
    std::cout << "[Multiprocessor] Test 5: Thread 3 finished running" << std::endl;
}
...
void thread2(uintptr_t)
{
    ...
    assert(local_sum == 20);
    std::cout << "[Multiprocessor] Test 5: Thread 2 finished running" << std::endl;
}
...
void thread1(uintptr_t)
{
    ...
    assert(local_sum == 50);
    std::cout << "[Multiprocessor] Test 5: Thread 1 finished running" << std::endl;
}

void thread_manager(uintptr_t)
{
    std::cout << "[Multiprocessor] Test 5: Running test with 3 threads" << std::endl;

    thread t1(thread1, 0);
    t1.join();

    assert(global_sum == (50 + 20 + 20)); // 50 from test_func1, 20 from test_func2, 20 from test_func3
    std::cout << "[Multiprocessor] Test 5: All threads finished." << std::endl;
}
