...
void test_func4(uintptr_t)
{
    ...
    thread::yield();
    assert(sum == 20);
    std::cout << "[Multiprocessor] Multiple Threads: Thread 4 finished running" << std::endl;
...
void test_func5(uintptr_t)
{
    ...
    thread::yield();
    assert(sum == 90);
    std::cout << "[Multiprocessor] Multiple Threads: Thread 5 finished running" << std::endl;
}

void test(uintptr_t)
{
    std::cout << "[Multiprocessor] Multiple Threads: Running test with 5 threads" << std::endl;
    thread(test_func1, 0);
    thread(test_func2, 0);
}
